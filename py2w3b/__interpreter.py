...
class html_translate():
    ...
    def compile(self, elements:list, route:str, pages:dict):
        """
        ...
        for index, element in enumerate(elements):
            ...
            if prefix in self.funcs:
                ...
                # Rendering if .show() is called
                
                print(real_list[len(real_list)-1])
                
                
                if real_list[len(real_list)-1] == True:
                    _interpretation = self.funcs[prefix](real_list)
                    try:
                        pages[route]["template"][_interpretation[0].lower()][index] = _interpretation[-1]
                    except:
                        pages[route]["template"][_interpretation[0].lower()].append(_interpretation[-1])
                    
                else:
                    del pages[route]["template"]["html"][index]
...
def translate(pages):
    ...
    for route in pages:
        ...
        translator.compile(page["html"], route, pages)
